...
For real-agent evals, set `WAYMARK_AGENT_EVAL_KEYS` in your local `.env` as a comma-separated or newline-separated list of Gemini API keys, then run `npm run test:agent:live`. The runner keeps the local mock Drive/Sheets app, but sends real model requests so it can score response quality, Waymark link usage, tool side effects, and multi-sheet behavior.
