...
> Set CLAUDE_MD_LOCKED=true to prevent regeneration. Updated: Thu Jun 18 16:18:08 UTC 2026
...
- **`@qa-worker`**: Autonomous Eval/QA patrol agent for the Waymark dev fleet. Polls the workboard for QA-stage items, validates each one via code review and E2E tests, posts a structured verdict, and loops forever. Runs persistently in the waymark-eval-worker Docker container.
...
- **`@waymark-qa`**: Pre-human QA validation agent for the Waymark dev fleet. Polls the workboard for tasks in the QA stage, runs code review and E2E Playwright tests, writes a structured verdict, and posts it back to the workboard. Operates as Quinn in the Agent Registry. Does not merge or close tasks — only posts verdicts so the human can make the final call.
