...
const OFF_OFFERS   = 1;
const OFF_ANSWERS  = 2;
const ALIVE_TTL      = 120_000;  // ms — generous TTL avoids premature eviction during API quota bursts
const POLL_MS        = 7_000;    // ms — reduced from 5s to lower Sheets API quota pressure
const HEART_MS       = 20_000;   // ms — increased from 15s; ALIVE_TTL/HEART_MS ratio gives 6 missed heartbeats
...
export class SheetWebRtcPeer {
    constructor({ sheetId, auth, getToken, peerId, displayName, iceServers, onMessage, onConnect, bufferFile }) {
        ...
        this._iceServers   = iceServers || STUN_SERVERS;
     ...
     * On DISCONNECTED: wait ICE_DISCONNECT_GRACE_MS then attempt ICE restart
     * before tearing down.  ICE restart is faster than full teardown + new
     * handshake because it reuses the existing DTLS session.
     * Immediate close on FAILED/CLOSED so next poll rebuilds quickly.
     */
    /** Build a diagnostic string from the current ICE candidate pair + transport stats. */
    async _iceDigest(pc, remotePeerId) {
        try {
            const parts = [];
            const iceConn = pc.iceTransports?.[0]?.connection;
            if (!iceConn) return "no-ice-transport";

            parts.push(`iceState=${iceConn.state}`);

            // Nominated (selected) pair
            const nom = iceConn.nominated;
            if (nom) {
                const lc = nom.localCandidate ?? nom.local;
                const rc = nom.remoteCandidate ?? nom.remote;
                parts.push(
                    `nominated-local=${lc?.type ?? "?"}/${lc?.protocol ?? "?"}/${lc?.host ?? lc?.address ?? "?"}:${lc?.port ?? "?"}`,
                    `nominated-remote=${rc?.type ?? "?"}/${rc?.protocol ?? "?"}/${rc?.host ?? rc?.address ?? "?"}:${rc?.port ?? "?"}`,
                );
            } else {
                parts.push("nominated=none");
            }

            // All candidate pairs in check list
            const checkList = iceConn.checkList ?? [];
            for (const pair of checkList) {
                const lc = pair.localCandidate ?? pair.local;
                const rc = pair.remoteCandidate ?? pair.remote;
                parts.push(
                    `pair[${pair.state ?? "?"}] local=${lc?.type ?? "?"}/${lc?.protocol ?? "?"}/${lc?.host ?? lc?.address ?? "?"}:${lc?.port ?? "?"} ` +
                    `remote=${rc?.type ?? "?"}/${rc?.protocol ?? "?"}/${rc?.host ?? rc?.address ?? "?"}:${rc?.port ?? "?"}`
                );
            }

            // DTLS state
            const dtls = pc.dtlsTransports?.[0];
            if (dtls) parts.push(`dtls=${dtls.state}`);
    ...
    _monitorIce(pc, remotePeerId) {
        ...
        pc.oniceconnectionstatechange = () => {
            ...
            // Dump diagnostic stats on any non-trivial state change
            if (state === "disconnected" || state === "failed" || state === "closed" ||
                state === "connected" || state === "completed") {
                this._iceDigest(pc, remotePeerId).then(digest => {
                    this._log(`ICE ${remotePeerId} diag: ${digest}`);
                }).catch(() => {});
            }
            if (state === "disconnected") {
                // Give ICE a grace window to self-heal (path change, NAT rebind, WiFi blip).
                // If still disconnected after the grace period, close for full rebuild
                // (the poll loop will rediscover the remote heartbeat and create a fresh offer).
                disconnectTimer = setTimeout(() => {
                    ...
                    if (entry?.pc === pc) {
                        ...
                        // Schedule immediate rebuild (skipped if poll already in progress)
                        setTimeout(() => {
                            this._poll().catch(e => this._log(`rebuild poll error: ${e.message}`));
                        }, 100);
                    }
                ...
                // Flush any notifications that were buffered while ICE was
                // disconnected — the DataChannel stays open across self-healed
                // disconnects so dc.onopen won't re-fire to trigger flush.
                const reconnEntry = this.peers.get(remotePeerId);
                if (reconnEntry?.dc?.readyState === "open") {
                    this._flushNotifQueue(remotePeerId, reconnEntry.dc);
                }
            } else if (state === "failed") {
                // ICE FAILED — close immediately and rebuild.  On LTE, DISCONNECTED
                // transitions to FAILED in <1s (consent checks fail when NAT rebinds).
                // Waiting for the grace timer wastes time; close now so the poll loop
                // can establish a fresh connection on the next cycle (~7s).
                clearTimeout(disconnectTimer);
                disconnectTimer = null;
                if (!this.destroyed) {
                    const entry = this.peers.get(remotePeerId);
                    if (entry?.pc === pc) {
                        this._log(`ICE ${remotePeerId} FAILED — closing for immediate rebuild`);
                        this._closeOne(remotePeerId);
                        setTimeout(() => {
                            this._poll().catch(e => this._log(`rebuild poll error: ${e.message}`));
                        }, 100);
     ...
     * Creates a new offer with iceRestart: true, writes it to the signaling
     * sheet, and waits for ICE to reconnect.  Falls back to full teardown
     * if the restart doesn't succeed within a timeout.
     */
    async _attemptIceRestart(pc, remotePeerId) {
        const entry = this.peers.get(remotePeerId);
        ...
        try {
            ...
            // Write the restart offer to our signaling row
            const vals = await this._readAll();
            let myOffers = this._parseJson(vals[this.block + OFF_OFFERS]) || {};
            myOffers[remotePeerId] = { sdp: pc.localDescription.sdp, ts: Date.now() };
            await this._writeOffers(myOffers);
            this._log(`ICE restart offer written for ${remotePeerId}`);

            // Wait for ICE to reconnect using polling — do NOT override
            // pc.oniceconnectionstatechange here because that would clobber
            // the _monitorIce handler and break future disconnect detection.
            const deadline = Date.now() + 30_000;
            while (Date.now() < deadline) {
                const ice = pc.iceConnectionState;
                if (ice === "connected" || ice === "completed") {
                    // Verify a candidate pair was actually nominated — werift can
                    // report "completed" when the remote already tore down its PC
                    // (no candidate pair selected despite successful state).
                    const iceConn = pc.iceTransports?.[0]?.connection;
                    if (!iceConn?.nominated) {
                        throw new Error(`ICE ${ice} but no nominated pair — remote likely dead`);
                    }
                    this._log(`ICE restart succeeded for ${remotePeerId}`);
                    return;
                }
                if (ice === "failed" || ice === "closed") {
                    throw new Error(`ICE restart failed: ${ice}`);
                }
                await new Promise(r => setTimeout(r, 500));
            }
            throw new Error("ICE restart timeout");
        } catch (e) {
            throw e; // caller handles fallback to full rebuild
        }
    }
     ...
     *
     * Also returns true when any existing peer has an ICE restart in flight
     * (ICE state checking or disconnected with grace timer pending).  Without
     * this, rebuildPeer() races with _attemptIceRestart(): the health timer
     * sees connectedPeers()===0 (because connectedPeers filters out
     * disconnected ICE), starts the STALE_PEER_MS clock, and tears down the
     * entire SheetWebRtcPeer while the ICE restart is still negotiating.
     */
    hasActiveHandshakes() {
        for (const [, entry] of this.peers) {
            if (entry.state === "connecting") return true;
            // ICE restart in progress on an existing peer — don't nuke it
            const ice = entry.pc?.iceConnectionState;
            if (ice === "checking" || ice === "disconnected") return true;
        }
        return false;
    }
