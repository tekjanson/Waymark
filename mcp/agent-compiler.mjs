...
/* ============================================================
   ...
   Exposes three MCP tools:
     agent_compile    — compile only when inputs change (hash-based), skip if up to date
     agent_list       — inventory: compiled vs registry vs missing
     agent_invalidate — delete a compiled agent + sidecar files to force recompile
...
/**
 * Compute a SHA-256 hash of the three inputs that determine a compiled agent:
 *   1. base.md.tmpl content
 *   2. domain-knowledge/{key}.md content (empty string if missing)
 *   3. The template's registry entry (serialised)
 * The hash is stored as a sidecar file (.github/agents/evals/{key}.sha) so
 * agent_compile can skip work when nothing has changed.
 */
function computeInputHash(templateKey) {
  const registry = loadRegistry();
  const tmpl = registry.templates.find(t => t.key === templateKey);
  if (!tmpl) throw new Error(`Template key "${templateKey}" not found in registry.`);
...
function toolAgentCompile(args) {
  ...
  const outputFile = path.join(OUTPUT_DIR, `waymark-${templateKey}.agent.md`);
  const hashFile   = path.join(EVAL_DIR, `${templateKey}.sha`);

  // Compute hash of current inputs before anything else
  let currentHash;
  try {
    currentHash = computeInputHash(templateKey);
  } catch (err) {
    return { error: err.message };
  }

  // Skip if compiled and inputs are unchanged (unless force)
  if (!force && existsSync(outputFile)) {
    ...
    if (storedHash === currentHash) {
      return {
        ...
        skipped:            true,
        upToDate:           true,
        message:            `Agent is up to date — inputs unchanged. Pass force:true to force recompile.`,
      };
    }
  }
  ...
  try {
    ...
    // Persist input hash so future calls can detect changes
    mkdirSync(EVAL_DIR, { recursive: true });
    writeFileSync(hashFile, currentHash, "utf8");
    return {
      ...result,
      message: result.wasAlreadyCompiled
        ? `Recompiled waymark-${templateKey} (inputs changed${force ? ", forced" : ""}).`
...
function toolAgentInvalidate(args) {
  ...
  // Remove sidecar files so the next agent_compile starts completely fresh
  const hashFile = path.join(EVAL_DIR, `${templateKey}.sha`);
  const evalFile = path.join(EVAL_DIR, `${templateKey}.eval.json`);
  if (existsSync(hashFile)) unlinkSync(hashFile);
  if (existsSync(evalFile)) unlinkSync(evalFile);

  return {
    deleted:  true,
    filePath: outputFile,
    message:  `Deleted waymark-${templateKey}.agent.md and sidecar files. Run agent_compile to rebuild.`,
  };
}
...
const server = new Server(
  { name: "waymark-agent-compiler", version: "2.0.0" },
  { capabilities: { tools: {} } }
);
  ...
  tools: [
    {
      ...
      description:
        ...
        "Skips silently when inputs (base template + domain knowledge + registry entry) are " +
        "unchanged — detected via SHA-256 hash sidecar. Set force:true to bypass the hash check.",
      inputSchema: {
        type: "object",
        properties: {
          templateKey: {
