...
description: Persistent build agent that queries the Waymark Workboard Google Sheet for To Do items, picks them up automatically, implements features in a single mega branch following AI_LAWS, writes isolated E2E tests, and loops forever waiting for the next task. All work stays local — never pushes.
...
2. **WORK** — Implement the highest-priority To Do item (full cycle: implement → test → commit on mega branch → mark QA on workboard). No pushing.
...
14. **Report results** — tell the user what was built, test count, commit hash, Drive test report link, and that it's ready for QA
