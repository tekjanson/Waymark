...
const definition = {
  ...
  /** Build a single checklist-row element */
  _buildRow(row, rowIdx, cols, template) {
  ...
  render(container, rows, cols, template) {
    /* If no category column, render flat — backward compatible */
    if (cols.category < 0) {
      for (let i = 0; i < rows.length; i++) {
        container.append(template._buildRow(rows[i], i + 1, cols, template));
      }
      return;
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      const cat = cell(rows[i], cols.category) || 'Other';
      if (!groups.has(cat)) groups.set(cat, []);
      groups.get(cat).push({ row: rows[i], originalIndex: i });
    }

    for (const [cat, items] of groups) {
      /* Section header */
      container.append(el('div', { className: 'checklist-group-header' }, [cat]));

      /* Group container for indented items */
      const groupEl = el('div', { className: 'checklist-group' });
      for (const { row, originalIndex } of items) {
        groupEl.append(template._buildRow(row, originalIndex + 1, cols, template));
      }
