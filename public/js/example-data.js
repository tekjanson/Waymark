...
export const EXAMPLE_SHEETS = {
  ...
  'Europe Trip': {
    folder: 'Travel',
    headers: ['Activity', 'Date', 'Location', 'Booking', 'Booking Link', 'Booking Details', 'Cost'],
    rows: [
      ['Flight to Paris',    '2026-06-15', 'JFK → CDG',         'AA-1234',  'https://aa.com/booking/AA-1234',          'Departs 10:30 AM. Check in online 24h before.',     '$650'],
      ['Hotel Le Marais',    '2026-06-15', 'Paris',             'BK-5678',  'https://booking.com/hotel/le-marais',     'Check-in from 3 PM. Breakfast included.',           '$180/night'],
      ['Eiffel Tower Tour',  '2026-06-16', 'Paris',             'TK-9012',  'https://toureiffel.paris/en',             'Summit access. Arrive 15 min early.',               '$35'],
      ['Louvre Museum',      '2026-06-17', 'Paris',             'TK-9013',  'https://louvre.fr/en/visit',              'Time slot: 10:00 AM. Audio guide included.',         '$20'],
      ['Train to Amsterdam', '2026-06-18', 'Paris → Amsterdam', 'TH-3456',  'https://thalys.com/booking/TH-3456',      'Departs Gare du Nord 09:22 AM. Seat 24B.',          '$120'],
      ['Canal Hotel',        '2026-06-18', 'Amsterdam',         'BK-7890',  'https://booking.com/hotel/canal-hotel',   'Check-in from 2 PM. Canal view room.',              '$160/night'],
      ['Van Gogh Museum',    '2026-06-19', 'Amsterdam',         'TK-2345',  'https://vangoghmuseum.nl/en/visit',       'Timed entry 11:00 AM. No bag check needed.',        '$22'],
      ['Bike Tour',          '2026-06-20', 'Amsterdam',         'TK-6789',  'https://bikecity.nl/tours',               'Meet at Centraal Station east entrance at 10 AM.',  '$45'],
      ['Flight Home',        '2026-06-21', 'AMS → JFK',         'KL-5678',  'https://klm.com/booking/KL-5678',         'Departs 2:15 PM. Online check-in opens 30h before.','$700'],
    ],
  },
  ...
  'Tokyo Trip': {
    ...
    headers: ['Activity', 'Date', 'Location', 'Booking', 'Booking Link', 'Booking Details', 'Cost'],
    rows: [
      ['Flight to Tokyo',      '2026-09-01', 'LAX → NRT',  'JL-100',   'https://jal.com/booking/JL-100',        'Departs 11:00 AM. Meal included.',                '$1,100'],
      ['Shinjuku Hotel',       '2026-09-01', 'Tokyo',       'BK-4321',  'https://booking.com/hotel/shinjuku',    'Check-in from 3 PM. Free WiFi.',                  '$120/night'],
      ['Senso-ji Temple',      '2026-09-02', 'Asakusa',     '',         '',                                      'Open 6 AM–5 PM. No booking required.',            'Free'],
      ['Tsukiji Market Tour',  '2026-09-03', 'Chuo',        'TK-1111',  'https://tsukijitour.com/TK-1111',       'Meet at main gate 7:30 AM.',                      '$60'],
      ['Day trip to Hakone',   '2026-09-04', 'Hakone',      'TK-2222',  'https://odakyu.jp/tour/TK-2222',        'Departs Shinjuku 8:00 AM. Mt Fuji views.',        '$80'],
      ['Return Flight',        '2026-09-05', 'NRT → LAX',  'JL-101',   'https://jal.com/booking/JL-101',        'Departs 4:30 PM. Check in 2h before.',            '$1,100'],
    ],
  },
  ...
  'Kitchen Compost Guide': {
    folder: 'Guides',
    headers: ['Guide', 'Slide', 'Objective', 'Instruction', 'Visual', 'Duration', 'Status'],
    rows: [
      ['Kitchen Compost Setup', 'Start with the right bin', 'Create a frictionless compost drop zone', 'Choose a small lidded container and keep it where prep scraps already happen so composting becomes the default path.', 'Countertop bin photo', '2 min', 'Done'],
      ['Kitchen Compost Setup', 'Know what goes in', 'Reduce sorting mistakes', 'Add fruit peels, coffee grounds, tea leaves, eggshells, and vegetable scraps. Keep the list visible near the station.', 'Green ingredient checklist', '1 min', 'Ready'],
      ['Kitchen Compost Setup', 'Avoid contamination', 'Protect the final compost stream', 'Keep meat, dairy, oily leftovers, and look-alike plastic bags out of the compost bin so the pickup stays clean.', 'Red do-not-add list', '1 min', 'Draft'],
      ['Weekly Bin Reset', 'Carry to the curb cart', 'Empty the bin before moisture builds up', 'Transfer the countertop scraps to the outdoor compost cart on the same day each week so the station never becomes a cleanup chore.', 'Arrow path to outdoor cart', '3 min', 'In Progress'],
      ['Weekly Bin Reset', 'Rinse and dry', 'Reset the station for the next cycle', 'Quickly rinse the small bin, let it dry, and drop in a fresh liner or newspaper layer before returning it to the counter.', 'Sink rinse illustration', '2 min', 'Ready'],
      ['Weekly Bin Reset', 'Post the reminder card', 'Make the process self-serve for housemates', 'Tape a short card above the station that shows what is compostable and when the curb cart gets emptied.', 'Printable reminder card', '1 min', 'Done'],
    ],
  },
  ...
  'Waymark Notifications': {
    ...
    headers: ['Title', 'Message', 'Type', 'Status', 'Icon', 'Priority', 'Created', 'Expires', 'Source', 'Sheet'],
    rows: [
      ['P0 Task Assigned',       'Critical task "Fix auth token expiry" has been assigned and is blocking the release.', 'alert',   'Active',    '🔴', 'High',   '2026-03-17T10:30:00', '',             'Kanban Board',  'sheet-028'],
      ['Budget Over Limit',      'March expenses ($4,250) exceed monthly budget ($4,000) by $250.',                      'warning', 'Active',    '💸', 'High',   '2026-03-16T09:00:00', '',             'Budget Tracker','sheet-016'],
      ['Overdue Tasks Detected', '3 tasks in Waymark Development board are past their due date.',                        'warning', 'Active',    '⏰', 'High',   '2026-03-17T08:00:00', '',             'Kanban Monitor','sheet-028'],
      ['Deployment Successful',  'Production deploy completed. 3 new features are now live for all users.',              'info',    'Read',      '✅', 'Medium', '2026-03-17T12:00:00', '',             'CI/CD Pipeline',''],
      ['Security Scan Complete', 'Weekly dependency audit found 0 critical vulnerabilities.',                            'info',    'Read',      '🔒', 'Low',    '2026-03-15T08:00:00', '',             'Security Bot',  ''],
      ['Team Meeting Reminder',  'Sprint review at 9 AM in Conference Room B. Bring your demo items.',                   'info',    'Dismissed', '📅', 'Medium', '2026-03-15T14:00:00', '2026-03-16',   'Calendar',      ''],
      ['Sync Error Resolved',    'Google Drive sync error from yesterday has been automatically resolved.',               'info',    'Dismissed', '🔄', 'Low',    '2026-03-14T11:00:00', '',             'Sync Service',  ''],
      ['Recipe Import Complete', '12 recipes were successfully imported from the shared cookbook spreadsheet.',           'info',    'Dismissed', '🍽️', 'Low',    '2026-03-13T15:30:00', '',             'Import System', ''],
    ],
  },
