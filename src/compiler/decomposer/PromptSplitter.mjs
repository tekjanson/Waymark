...
export class PromptSplitter {
    ...
    async #buildSharedManifest(orderedUnits, pathMap) {
        ...
        return [
            ...
            ...siblingImports,
            '',
            '## Test Harness Contract (two-tier)',
            '  TIER 1 — Unit test (your .test.mjs):',
            '    - Tests ONLY the single function this file exports.',
            '    - Sibling deps are mocked via vi.mock() with contract-derived stubs.',
            '    - Stubs match the AstParser contract — param count, not behaviour.',
            '    - Your implementation must satisfy the unit test with REAL logic.',
            '    - Do not design your implementation to match the mock return values.',
            '  TIER 2 — Integration smoke (runs after all units compile):',
            '    - Imports real compiled units from the barrel (index.mjs).',
            '    - No mocks. Validates boundaries do not throw at runtime.',
            '    - You do not need to write or think about this — it is auto-generated.',
            '',
